	...
	def autonomous(self):
		...
		while self.isAutonomous() and self.isEnabled():
			if(self.autoSwitch.get()):
				...
				#begin auto test code#	
				while(tm.clock() <= (begin + 4)):
					self.motorFrontRight.set(.5)
					self.motorBackRight.set(.5)
					self.motorFrontLeft.set(-.5)
					self.motorBackLeft.set(-.5)
				while(tm.clock() <= (begin + 10)):
					self.motorFrontRight.set(.3)
					self.motorBackRight.set(.6)
					self.motorFrontLeft.set(-.7)
					self.motorBackLeft.set(-.7)
